...
use simulator::simple::node_builder::NodeBuilderTrait;
use simulator::{
...
fn spawn_simulation_builder(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    new_builder
        ...
        .for_each(|(i, n_builder)| {
            ...
            //    return;
            //}
            match (**n_builder).lock().unwrap().generate_graphics_info() {
                ...
                node::graphics::Info::Street(ninfo) => {
                    ...
                    let target_street = Arc::clone(&new_builder.nodes[i]);
                    println!("   Set target street");
                    new_builder
                        .nodes
                        .iter()
                        .enumerate()
                        .filter(|(j, n)| {
                            (!target_street.lock().unwrap().is_connected(*n))
                                && (**n).lock().unwrap().is_connected(&target_street)
                        })
                        .for_each(|(j, _n)| {
                            println!("printäng j: {}", j);
                            println!("paefijöejsfk");
                            for c in (*target_street.lock().unwrap()).get_connections().iter() {
                                let index_target = (*target_street).lock().unwrap().get_id();
                                let index_conn = (*c.upgrade().unwrap()).lock().unwrap().get_id();
                                assert_ne!(index_target, index_conn);
                                assert!(!ptr::eq(c.as_ptr(), &*target_street));
                            }

                            (*target_street).lock().unwrap().get_connections()[0]
                                .upgrade()
                                .unwrap()
                                .lock()
                                .unwrap()
                                .get_weight();
                            println!(
                                "Hello. Calling from after infinite deadlock. Have large weight"
                            );
                            let end_index = (*(*target_street).lock().unwrap().get_connections()
                                [0]
                            .upgrade()
                            .unwrap()
                            .lock()
                    ...
                    println!("Generated all se nodes graphical things");
                    drop(target_street);
                }
            }
        });
