use bevy::{ecs::schedule::ShouldRun, prelude::{Res, ResMut, QuerySet, Query, Entity, With, Commands, DespawnRecursiveExt, MouseButton, Transform}, input::Input, window::Windows, math::Vec2};
use bevy_prototype_lyon::entity::ShapeBundle;
use simulator::nodes::NodeBuilderTrait;

use crate::{UIState, toolbar::ToolType, sim_manager::SimManager, SimulationID, NodeBuilderRef, NodeType, UnderCursor, themes::UITheme, Camera, node_bundles::node_render, NeedsRecolor};
use crate::input;
...
pub fn delete_node_system(
    ...
    if let Ok((sim_id, )) = shape_queries.q0().single() {
        if let Ok(sim_builder) = sim_manager.modify_sim_builder() {
            ...
            let removed_nodes = sim_builder.remove_node_and_connected_by_id(sim_id.0).expect("Unable to remove node");
...
pub fn select_node(
    ...
    let (entity, _, _) = match input::get_shape_under_mouse(mouse_click, windows, shapes.q0(), &camera) {
        Some(s) => s,
        None => return,
    };
    if let Ok(prev_selected) = shapes.q1().single() {
        commands.entity(prev_selected).remove::<SelectedNode>().insert(NeedsRecolor);
    }
    commands.entity(entity).insert(SelectedNode).insert(NeedsRecolor);
        ...
        match tool.get_type() {
            ...
            ToolType::Select => {
                ...
                if let Some(pos) = click_pos {
                    let (entity, sim_id, transform,  nbr, node_type ) = match  shape_queries.q0().single() {
                        Ok(result) => result,
                        Err(_) => return,
                    };
